...
function buildProductCatalogPrompt(products: ProductCatalogCandidate[]): string {
  ...
  return [
    ...
    "- quantity_estimate is the number of PLATES or SERVINGS of the dish visible in the image.",
    "- Do NOT count individual pieces of food on a plate. A single plate with multiple pieces (e.g. 3 chicken wings on one plate) is quantity 1.",
    "- Each distinct plate, bowl, or serving container counts as 1 serving regardless of how many individual food items are on it.",
    "- If the image shows 2 separate plates of the same dish, quantity_estimate is 2.",
    "- When in doubt, default to 1.",
    "- If uncertain about the product, pick the closest valid catalog product and reduce confidence.",
    "Product Catalog:",
    productList,
  ].join("\n");
}
